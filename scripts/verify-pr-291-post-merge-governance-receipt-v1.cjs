...
const docPath = "docs/governance/PR_291_POST_MERGE_GOVERNANCE_RECEIPT_V1.md";
const receiptPath = "receipts/governance/pr-291-post-merge-governance-receipt-v1.json";
function fail(message) { console.error("FAIL pr-291-post-merge-governance-receipt-v1: " + message); process.exit(1); }
...
if (receipt.artifact !== "pr-291-post-merge-governance-receipt-v1") fail("artifact mismatch");
if (receipt.pr !== 291) fail("PR mismatch");
if (receipt.merged_artifact !== "v2-operator-unpark-approval-candidate-v1") fail("merged artifact mismatch");
...
const requiredText = [
  ...
  "final operator unpark approval receipt, not command execution"
];
...
const trueFlags = [
  ...
  "operator_unpark_approval_candidate_merged",
...
console.log("PASS pr-291-post-merge-governance-receipt-v1");
