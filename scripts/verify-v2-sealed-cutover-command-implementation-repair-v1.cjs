...
for (const text of ["QPF_MAINNET_CUTOVER_EXECUTE","--require-command-hash","No deployment executed","No broadcast executed","No wallet signing executed","No state-changing transaction executed","live execution body intentionally not implemented"]) if (!src.includes(text)) fail("missing safety text: " + text);
