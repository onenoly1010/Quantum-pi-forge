...
class AlignmentProofSubmitter:
    ...
    def submit_alignment_proof(
        self,
        proof_hash: bytes,
        alignment_score: float,
        task_type: str = "financial_oracle",
        drift_resolved: bool = False
    ) -> Optional[str]:
        try:
            if not isinstance(proof_hash, (bytes, bytearray)) or len(proof_hash) != 32:
                raise ValueError("proof_hash must be exactly 32 bytes")

            if not 0.0 <= float(alignment_score) <= 1.0:
            ...
            logger.info(
                f"✅ ALIGNMENT PROOF SUBMITTED: {tx_hash.hex()[:16]}... | Score: +{alignment_score:.2f}"
            )
            return tx_hash.hex()
...
def claim_onchain_reward(proof_hash: bytes, alignment_score: float) -> bool:
    try:
        node_id = int(os.environ.get("ZERO_G_NODE_TOKEN_ID", "0"))
        private_key = os.environ.get("ZERO_G_WALLET_PRIVATE_KEY", "")
...
if __name__ == "__main__":
    ...
    print("  export ZERO_G_NODE_TOKEN_ID=YOUR_NODE_NFT_ID")
    print("  export ZERO_G_WALLET_PRIVATE_KEY=YOUR_WALLET_KEY")
    print()
    print("Call claim_onchain_reward(proof_hash, score) from your main loop")
    print()
    print("⟨OO⟩ READY TO ALIGN ON ARISTOTLE")
